...
class Server {
  constructor(customConfig = {}) {
    ...
    process.stdout.write(`${chalk.inverse('CONFIG:')} \n${JSON.stringify(this.config, null, 2)}\n`)
  }
  start() {
    const server = http.createServer(async (req, res) => {
      ...
      const filePath = path.join(this.config.root, req.url) // !file absolute path
      try {
        ...
        if (stats.isFile()) {
          ...
          // test isFresh
          if (this.config.cacheMode && isFresh(stats, req, res)) {
            res.statusCode = 304
            res.end()
            return
          }
          ...
          if (this.config.compress && ext.match(this.config.fileToCompress)) {
    ...
    server.listen(this.config.port, this.config.host, () => {
      ...
      process.stdout.write(`server running at ${chalk.black.bgGreen(url)}`)
      if (this.config.autoOpenUrl) autoOpenUrl(url) // auto open url link on browser
    })
  }
}
